...
public class PurchaseService {
    ...
    private Purchase toEntity(Integer supplierId, Integer employeeId, PurchaseRequestDTO dto) {
        ...
        Supplier supplier = supplierRepository.findById(supplierId)
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Supplier not found with id: " + supplierId));
    ...
    public PurchaseResponseDTO confirmPurchase(Integer id) {
        ...
        if (purchase.getStatus() != Status.PENDENTE) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Purchase is not pending with id: " + id);
        }
    ...
    public PurchaseResponseDTO cancelPurchase(Integer id) {
        ...
        if (purchase.getStatus() == Status.RECEBIDA) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Cannot cancel received purchase with id: " + id);
        }
        if (purchase.getStatus() == Status.CANCELADA) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Purchase already canceled with id: " + id);
        }
